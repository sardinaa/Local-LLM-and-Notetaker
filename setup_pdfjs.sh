...
#
# Usage: ./setup_pdfjs.sh [--force]
#   --force: Force reinstallation even if PDF.js already exists
...
if [ -f "$PDFJS_DIR/build/pdf.js" ] && [ "$FORCE_REINSTALL" = false ]; then
    echo "PDF.js core files are already present."
    echo "Checking and applying custom configurations..."
    
    # Still apply custom configurations even if PDF.js exists
    if [ -d "pdfjs-config" ]; then
        ...
        if [ -f "pdfjs-config/highlight-plugin.js" ]; then
            ...
            echo "Applied highlight-plugin.js"
        fi
        
        # Copy customized viewer files to web directory
        if [ -f "pdfjs-config/viewer.html" ]; then
            cp pdfjs-config/viewer.html "$PDFJS_DIR/web/"
            echo "Applied customized viewer.html"
        fi
        
        if [ -f "pdfjs-config/viewer.css" ]; then
            cp pdfjs-config/viewer.css "$PDFJS_DIR/web/"
            echo "Applied customized viewer.css"
        fi
        
        # Copy custom images
        if [ -d "pdfjs-config/images" ]; then
            cp pdfjs-config/images/* "$PDFJS_DIR/web/images/" 2>/dev/null || true
            echo "Applied custom images"
        fi
        
        echo "Custom configurations applied to existing installation."
    fi
    
    # Skip to verification
    echo "Skipping download since PDF.js is already installed."
else
    # Download PDF.js (fresh install or forced reinstall)
    if [ "$FORCE_REINSTALL" = true ]; then
        echo "Force reinstall requested. Removing existing PDF.js installation..."
        rm -rf "$PDFJS_DIR"
        mkdir -p "$PDFJS_DIR"
    fi
