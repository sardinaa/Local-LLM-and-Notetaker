...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            # Create indexes for shopping ingredients
            ...
            # Create trigger for shopping ingredients timestamps
            conn.execute('''
    ...
    def _create_shopping_table(self, conn):
        """Create the shopping ingredients table if it doesn't exist."""
        try:
            conn.execute('''
                CREATE TABLE IF NOT EXISTS shopping_ingredients (
                    id TEXT PRIMARY KEY,
            ...
            logging.error(f"Error creating shopping table: {e}")

    def add_shopping_ingredient(self, ingredient_name: str, recipe_name: Optional[str] = None, 
                              quantity: Optional[str] = None, unit: Optional[str] = None) -> Optional[str]:
        """Add an ingredient to the shopping list."""
        try:
            ingredient_id = str(uuid.uuid4())
            with self.get_connection() as conn:
                # Ensure shopping table exists
                cursor = conn.cursor()
                cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='shopping_ingredients'")
                if not cursor.fetchone():
                    self._create_shopping_table(conn)
    ...
    def add_shopping_ingredients_batch(self, ingredients: List[Dict]) -> List[str]:
    ...
    def get_shopping_ingredients(self) -> List[Dict]:
        """Get all shopping ingredients."""
        try:
            with self.get_connection() as conn:
    ...
    def get_shopping_ingredients_aggregated(self) -> List[Dict]:
        """Get shopping ingredients aggregated by ingredient name (summing quantities)."""
        try:
            with self.get_connection() as conn:
                # Check if table exists and create if it doesn't
                cursor = conn.execute("""
                ...
                for row in cursor.fetchall():
                    ...
                    if row_dict['recipe_count'] == 1:
                        recipes = row_dict['recipes'].split(',')
    ...
    def get_shopping_ingredients_by_recipe(self) -> Dict[str, List[Dict]]:
        """Get shopping ingredients grouped by recipe."""
        try:
            ingredients = self.get_shopping_ingredients()
            grouped = {}
            for ingredient in ingredients:
    ...
    def update_shopping_ingredient_checked(self, ingredient_id: str, checked: bool) -> bool:
        """Update the checked status of a shopping ingredient."""
        try:
            with self.get_connection() as conn:
                # Ensure shopping table exists
                cursor = conn.cursor()
    ...
    def delete_shopping_ingredient(self, ingredient_id: str) -> bool:
    ...
    def clear_shopping_ingredients(self, recipe_name: Optional[str] = None, checked_only: bool = False) -> int:
        """Clear shopping ingredients. If recipe_name provided, clear only ingredients from that recipe.
        If checked_only is True, clear only checked ingredients."""
        try:
            with self.get_connection() as conn:
                # Ensure shopping table exists
                cursor = conn.cursor()
