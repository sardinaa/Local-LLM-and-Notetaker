...
import app.integrations.jobspy_adapter as jobspy_adapter
...
class JobScraperService:
    ...
    def __init__(self, db_manager: DatabaseManager, data_service: DataService):
        ...
        self.jobs_repo = JobsRepository(db_manager.db_path)
    ...
    def _run_scheduled_scrape(self, config_id: str):
        ...
        try:
            configs = self.jobs_repo.get_scraper_configs()
    ...
    def _insert_job_if_new(self, job: JobMatch, config: Dict[str, Any]) -> bool:
        ...
        if job_result:
            ...
            if job.provenance:
                self.jobs_repo.save_job_provenance(job_id, job.provenance)
